...
public class Test2 {
	...
	public static void main(String[] args) {
		...
		/*
		 * 반복문을 사용하여 숫자와 마크를 중첩으로 반복
		 * => 마크 + 숫자를 결합한 문자열을 ArrayList 객체 (cards)에 추가
		 *    ex) "♠" 마크와 숫자 "2" 를 결합하여 "♠2" -> cards 에 추가
		 * => 최종적으로 생성된 트럼프 카드 목록(cards)을 출력    
		 * */
		for(String mark : marks) { // "♠", "◆", "♥", "♣"
			
			for(String number : numbers) { // "A", "2", "3", "4"
//				System.out.println(mark + number);
				cards.add(mark + number); 
			}
		}
		
		System.out.println(cards); // toString() 생략, 오버라이딩
		...
		System.out.println("player1의 카드: " + player1[0]);
		System.out.println("player2의 카드: " + player2[0]);
		System.out.println("player3의 카드: " + player3[0]);
		System.out.println(cards);
