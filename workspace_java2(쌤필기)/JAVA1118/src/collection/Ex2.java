...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 * 	   => ArrayList 는 멀티 쓰레드 환경을 지원하지 않음
		 * 
		 * < ArrayList vs LinkedList >
		 * - 기본적인 구조가 완전 다르며, 메서드 동일함
		 * - ArrayList는 배열 구조로써 인덱스를 활용하므로
		 *   데이터 탐색이나 순차적인 추가/삭제에 빠르다
		 * - LinkedList는 다음 데이터의 위치(주소)를 현재 데이터가 갖고 있는 형태이며
...
//		System.out.println("5번 인덱스 요소: " + list.get(5)); // 오류 발생!
		...
		System.out.println("===============================");
		System.out.println("list 객체에 모든 요소 출력 : " + list);
//		System.out.println("인덱스를 사용하여 인덱스 1 삭제 : " + list.remove(1)); 
//		System.out.println("정수 2를 지정하여 해당 요소 직접 삭제 : " + list.remove(2));
		...
		System.out.println("3이라는 요소의 인덱스 번호 : " + list.indexOf(10));
		
		System.out.println("실수 3.14가 존재하는가? " + list.contains(3.14)); 
		System.out.println("실수 5.0가 존재하는가? " + list.contains(5.0));
...
//		}
		...
		String[] str = {"1", "2", "3"};
		List arrayList3 = Arrays.asList(str);
...
//		System.out.println(set);
		...
		// -----------------------------------------------------------------------
		// 중복 제거
		Set set2 = new HashSet(Arrays.asList("ABCD", "A", "ABCD", "AB", "A", "AB", "ABC"));
		List list5 = new ArrayList(set2);
		System.out.println(list5);
		
		// 순서를 유지하는 LinkedHashSet
		Set set3 = new LinkedHashSet(Arrays.asList("ABCD", "A", "ABCD", "AB", "A", "AB", "ABC"));
