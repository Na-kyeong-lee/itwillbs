...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 *   예외 발생시 해결책을 기술하여 프로그램이 정상적으로 종료되도록 처리 가능
		 ...
		 *   => 예외가 발생하지 않으면 catch블록은 실행되지 않는다!     
		 * 
		 * - 예외 종류 분류
		 ...
		 * - java.lang 패키지 내에 Exception 클래스와 서브 클래스들이 제공되며
		...
		try {
			...
			System.out.println(num1 / num2); // 예외가 발생하는 코드
			...
			// JVM 에 의해 ArithmeticException 예외 객체가 생성되고 던져짐
			// 따라서, catch 블록의 소괄호 안에 해당하는 예외 클래스와 일치하는 참조변수를 선언하여
			// 던저지는 예외 객체를 전달받아야한다
		...
		// catch 블록 코드 실행 후 try ~ catch 문 밖으로 나와 나머지 코드를 정상적으로 실행함
		// => 즉, 예외가 발생하더라도 프로그램이 정상적으로 종료 되도록 처리됨
		...
		try {
			System.out.println("try 블록 시작!");
			int[] arr = {1, 2, 3};
			System.out.println(arr[2]); // ArrayIndexOutOfBoundsException 예외 발생!
			System.out.println("try 끝!"); // 예외 발생시 실행되지 못하는 코드
		...
		try {
			...
			System.out.println("try 블록 끝!"); // 예외 발생시 실행되지 못하는 코드
