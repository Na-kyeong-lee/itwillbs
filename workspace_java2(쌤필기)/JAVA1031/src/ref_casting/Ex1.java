...
public class Ex1 {
	...
	public static void main(String[] args) {
		 ...
		 * - 업캐스팅(Up casting)과 다운캐스팅(Down casting) 으로 분류함
		 * 
		 * 1. 업캐스팅 (Up casting)
...
//		long l = (long)a;
		...
		// 1. 참조데이터타입에서의 묵시정(자동)형변환 = 업캐스팅
		Child c = new Child();
		// Child 타입 참조변수로 접근 가능한 메서드 : 2개
		c.parentPrn();	// Parent 꺼
		c.childPrn();	// Child 꺼
		
		// 슈퍼클래스 타입 레퍼런스 변수 선언
		Parent p;
		p = c;	// 서브클래스 인스턴스 주소를 슈퍼클래스 타입 변수에 전달
		// => 슈퍼클래스 타입 변수(p)가 서브클래스 인스턴스(new C())를 가리킴
...
//		p3.childPrn();
		...
		// 따라서, 서브클래스 타입 c3로 접근가능한 메서드 : 2개
		c3.parentPrn();
		c3.childPrn();
		
		/*
		 * 다운 캐스팅 (Down casting) 결론
		 * - 서브클래스의 레퍼런스가 슈퍼클래스의 인스턴스를 참조하는 것
